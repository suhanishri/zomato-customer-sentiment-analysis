...
SEARCH_TOPIC = (
    ...
    f"service quality and competitor comparisons"
)

# tighter set than before to reduce quota burn
CANDIDATE_QUERIES = [
...
f"{BRAND_NAME} food quality complaint",
f"{BRAND_NAME} order cancelled refund",
...
META_PATTERNS = [
    ...
    r"\bvaluable information\b",
    r"\bhow to get fssai\b",
    r"\bfssai number\b",
    r"\bcustomer care ka number\b",
    r"\bemail kiska de\b",
    r"\bvideo banao\b",
]

COMPETITOR_TERMS = {"ondc", "paytm", "swiggy", "rapido", "blinkit", "zepto"}
COMPARISON_TERMS = {"better than", "worse than", "compared to", "vs", "more than", "less than"}


# ---------------- errors ----------------
class QuotaOrAccessError(Exception):
    pass
    ...
    for i in range(retries):
        try:
            ...
            if r.status_code == 403:
                raise QuotaOrAccessError(f"403 from YouTube API: {r.text[:500]}")
            r.raise_for_status()
            return r.json()
...
def keyword_score(row: Dict[str, Any]) -> float:
    text = normalize_text(" ".join([
        row.get("title", ""), row.get("description", ""), row.get("channel_title", ""),
        " ".join(row.get("tags", [])) if isinstance(row.get("tags"), list) else ""
    ]))
    score = 0.0
...
def run_discovery() -> None:
    ...
    if filtered:
        pd.DataFrame(filtered).to_csv(RAW / "youtube_filtered_out_videos_raw.csv", index=False)
...
def has_competitor_spillover_without_brand(text: str) -> bool:
    ...
    # drop if competitor/platform is mentioned but zomato is absent
    if has_comp and not has_brand and not has_comp_word:
        return True

    # drop if comment is mostly about another platform/helpdesk issue
...
def comment_keyword_relevance(text: str) -> int:
    t = normalize_text(text)
    return sum(1 for term in COMMENT_RELEVANCE_TERMS if term in t)
...
def is_relevant_comment(text: str) -> bool:
    ...
    # stronger requirement than before
    if BRAND_NAME in normalize_text(t) and rel >= 1:
        return True
...
def run_harvest() -> None:
    ...
    try:
        for idx, row in shortlist_df.iterrows():
            ...
            # cheaper mode: relevance for all, time only for top 5
            relevance_comments = fetch_comments(vid, MAX_COMMENTS_PER_VIDEO, "relevance")
            time_comments = fetch_comments(vid, MAX_COMMENTS_PER_VIDEO, "time") if idx < 5 else []
