...
\section{End-to-End Project Flow}

The project followed a structured end-to-end workflow beginning with Google Play Store review collection and ending with managerial interpretation through benchmarking and dashboard visualization. The flow diagram below summarizes how the major components of the study connect with one another.

\begin{figure}[H]
\centering
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=1.55cm and 0.55cm,
    every node/.style={font=\small},
    process/.style={draw, rounded corners=3pt, align=center, minimum width=3.0cm, minimum height=0.95cm, fill=gray!8},
    store/.style={draw, rounded corners=3pt, align=center, minimum width=3.2cm, minimum height=0.95cm, fill=blue!6},
    insight/.style={draw, rounded corners=3pt, align=center, minimum width=3.2cm, minimum height=0.95cm, fill=green!8},
    arrow/.style={-{Latex[length=3mm]}, thick}
]
\node[store] (collect) {Google Play Review\\Collection};
\node[process, right=of collect] (clean) {Cleaning, Relevance\\Filtering, Deduplication};
\node[store, right=of clean] (dataset) {Final Zomato\\Review Dataset};
\node[process, below=of collect] (label) {Rating-Based\\Sentiment Labeling};
\node[process, below=of clean] (theme) {Rule-Based Theme\\Classification};
\node[process, below=of dataset] (benchmark) {Benchmark Modeling\\and SentiLens Selection};
\node[process, below=of theme] (tables) {Theme Summaries,\\Recent Views, Trends};
\node[process, below=of benchmark] (dashboard) {Streamlit Dashboard\\and Comparison Views};
\node[insight, below=of dashboard] (insights) {Managerial Insights for\\Brand Strategy and\\Customer Experience};

\draw[arrow] (collect) -- (clean);
\draw[arrow] (clean) -- (dataset);
\draw[arrow] (dataset) -- (benchmark);
\draw[arrow] (collect) -- (label);
\draw[arrow] (clean) -- (theme);
\draw[arrow] (dataset) -- (theme);
\draw[arrow] (label) -- (tables);
\draw[arrow] (theme) -- (tables);
\draw[arrow] (benchmark) -- (dashboard);
\draw[arrow] (tables) -- (dashboard);
\draw[arrow] (dashboard) -- (insights);
\end{tikzpicture}%
}
\caption{End-to-end project flow from review collection to managerial interpretation.}
...
\section{SentiLens Model Architecture}

The final best-performing model in the project is SentiLens, which uses TF-IDF vectorization with Logistic Regression for sentiment prediction. The architecture is intentionally simple and interpretable, which makes it suitable for an MBA-oriented applied study where both accuracy and explainability matter.

\begin{figure}[H]
\centering
\resizebox{0.92\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=1.35cm and 0.55cm,
    every node/.style={font=\small},
    block/.style={draw, rounded corners=3pt, align=center, minimum width=2.8cm, minimum height=0.95cm, fill=orange!8},
    output/.style={draw, rounded corners=3pt, align=center, minimum width=3.0cm, minimum height=0.95cm, fill=green!8},
    arrow/.style={-{Latex[length=3mm]}, thick}
]
\node[block] (input) {Customer Review\\Text};
\node[block, right=of input] (prep) {Text Preprocessing\\and Standardization};
\node[block, right=of prep] (tfidf) {TF-IDF Feature\\Vectorization};
\node[block, right=of tfidf] (lr) {Logistic Regression\\Classifier};
\node[output, right=of lr] (sent) {Sentiment Output\\Positive / Neutral / Negative};
\node[block, below=of tfidf] (score) {Scoring Across Final\\Dataset};
\node[output, below=of sent] (use) {Dashboard, Theme Analysis,\\Benchmark Comparison};
...
\draw[arrow] (lr) -- (sent);
\draw[arrow] (lr) -- (score);
\draw[arrow] (score) -- (use);
\draw[arrow] (sent) -- (use);
\end{tikzpicture}%
}
\caption{SentiLens architecture used for final sentiment prediction and downstream analysis.}
\label{fig:sentilens-architecture}
\end{figure}
